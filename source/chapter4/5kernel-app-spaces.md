...
**注意**：`satp::set` 之后，所有的内存访问都会经过 MMU 转换。由于我们使用恒等映射，代码可以无缝继续执行。
...
| 概念 | 说明 |
|------|------|
| 恒等映射 | 虚拟地址 = 物理地址，用于内核地址空间 |
| ELF 解析 | 从 ELF 文件获取程序段信息 |
| Process | 包含 ForeignContext 和 AddressSpace |
| ForeignContext | 包含 LocalContext 和 satp |
| 传送门 | 同时映射到内核和应用地址空间的代码 |
| 地址空间隔离 | 每个进程只能访问自己的物理页 |
